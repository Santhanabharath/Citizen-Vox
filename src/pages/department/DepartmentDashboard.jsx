...
import { Activity, Map, AlertTriangle, CheckCircle, ShieldAlert, Bot, Users } from 'lucide-react';
  ...
  return (
    <div className="dashboard-container" style={{ padding: '2rem' }}>
      <header style={{ marginBottom: '2rem' }}>
        <h1 className="text-h1" style={{ color: 'var(--text-primary)' }}>Department Dashboard</h1>
        <p className="text-muted">Real-time overview for {user?.department || 'your department'}</p>
      </header>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem' }}>
        <section style={{ background: 'var(--surface)', padding: '1.5rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <h2 className="text-h3">Department Action Queue</h2>
            <Link to="/department/queue" style={{ color: 'var(--accent)', fontSize: '0.875rem', textDecoration: 'none' }}>View All →</Link>
          </div>
          {stats?.activeIssues === 0 ? (
            ...
            <div style={{ padding: '3rem', textAlign: 'center', background: 'var(--surface-hover)', borderRadius: 'var(--radius-md)' }}>
              <p className="text-muted">You have {stats?.activeIssues} active issues. Please assign workers from the Queue.</p>
              <Link to="/department/queue" className="btn-primary" style={{ marginTop: '1rem', display: 'inline-block', textDecoration: 'none' }}>Open Queue</Link>
            </div>
          )}
        </section>
        ...
        <section style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div style={{ background: 'var(--surface)', padding: '1.5rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)' }}>
            <h3 className="text-h3" style={{ marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Users size={18} /> Field Workers</h3>
            <p className="text-small text-muted" style={{ marginBottom: '1rem', lineHeight: '1.4' }}>
              Manage worker workload and assignments.
            </p>
            <Link to="/department/workers" className="btn-outline" style={{ display: 'block', textAlign: 'center', textDecoration: 'none' }}>View Workers</Link>
          </div>
        </section>
      </div>
    </div>
